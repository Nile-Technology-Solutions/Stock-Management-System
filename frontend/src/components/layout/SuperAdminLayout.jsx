...
const SuperAdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex relative overflow-x-hidden">
      ...
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-purple-500 text-white px-4 py-2 rounded-lg z-50">
        Skip to main content
      </a>

      {/* Mobile overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 md:hidden" onClick={() => setMobileMenuOpen(false)} />
      )}

      {/* Sidebar */}
      <div className={`${sidebarCollapsed ? 'w-20' : 'w-72'} bg-slate-900/95 dark:bg-slate-800/95 backdrop-blur-md transition-all duration-300 flex flex-col fixed md:relative inset-y-0 left-0 z-50 transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 border-r border-slate-800/50 dark:border-slate-700/50`}>
        {/* Sidebar Header */}
          ...
          <div className="relative flex items-center justify-between">
            {!sidebarCollapsed ? (
              <div className="flex items-center gap-3">
                <div className="relative group">
                  ...
                  <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-white p-1.5 shadow-lg shadow-purple-500/25 ring-2 ring-purple-500/30 group-hover:ring-purple-500/50 transition-all duration-300">
                    <img src="/src/assets/LOGO.png" alt="AddHomes Logo" className="w-full h-full object-contain" />
                  </div>
              ...
              <div className="relative group mx-auto">
                <div className="absolute inset-0 bg-gradient-to-r from-purple-400/20 to-pink-400/20 rounded-xl blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-white p-1.5 shadow-lg shadow-purple-500/25 ring-2 ring-purple-500/30 group-hover:ring-purple-500/50 transition-all duration-300">
                  <img src="/src/assets/LOGO.png" alt="SMS Logo" className="w-full h-full object-contain" />
                </div>
              </div>
            )}
            <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/50 transition-all duration-200 hidden md:block group" aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
              {sidebarCollapsed ? <ChevronRight className="w-5 h-5 group-hover:scale-110 transition-transform duration-200" /> : <ChevronLeft className="w-5 h-5 group-hover:scale-110 transition-transform duration-200" />}
            </button>
            <button onClick={() => setMobileMenuOpen(false)} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/50 transition-all duration-200 md:hidden" aria-label="Close menu">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <nav className="flex-1 p-3 sm:p-4 space-y-1 sm:space-y-2">
          {navigationItems.map((item) => (
            <Link key={item.name} to={item.href} className={`group flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 relative overflow-hidden ${isActiveRoute(item.href) ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg shadow-purple-500/25' : 'text-slate-300 hover:text-white hover:bg-slate-800/50'}`} aria-current={isActiveRoute(item.href) ? 'page' : undefined}>
              {isActiveRoute(item.href) && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700" />}
            ...
            <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-purple-500/10 border border-purple-500/20 rounded-xl">
              <div className="flex items-center">
                ...
                <div className="ml-3 min-w-0 flex-1">
                  ...
                  <span className="px-2 py-0.5 bg-purple-500 text-white rounded-full text-[10px] uppercase font-bold">{getRoleDisplayName(user?.role)}</span>
                </div>
              </div>
            </div>
          )}
          <button onClick={handleLogout} className={`w-full flex items-center px-4 py-3 text-sm font-medium text-slate-300 hover:text-white hover:bg-red-500/20 rounded-xl transition-all duration-200 group ${sidebarCollapsed ? 'justify-center' : ''}`} aria-label="System Logout">
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-white/30 dark:border-slate-700/40 px-4 sm:px-6 py-4 relative z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => setMobileMenuOpen(true)} className="md:hidden p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-white/20 dark:hover:bg-slate-700/20 transition-all duration-200" aria-label="Open menu">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{getCurrentPageTitle()}</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 hidden sm:block">Super Admin Console</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button className="relative p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-white/20 dark:hover:bg-slate-700/20 transition-all duration-200 group">
                ...
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-pink-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{notifications}</span>
              </button>
              ...
              <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-lg">
                ...
                <span className="text-xs font-bold text-purple-600 dark:text-purple-400 uppercase">Super Admin</span>
              </div>
            </div>
          </div>
        </header>
