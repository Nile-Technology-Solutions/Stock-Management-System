...
const DashboardCards = ({ data }) => {
  ...
  const MiniChart = ({ data, color }) => {
    ...
    if (safeData.length === 0) return <div className="h-8" />;
        ...
        <GlassCard key={index} variant="standard" hoverable className="group">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">
              {typeof stat.value === 'object' ? (stat.value?.label || stat.value?.amount || JSON.stringify(stat.value)) : stat.value}
            </p>
            <div className="flex items-center justify-between">
              <span className={`text-sm font-semibold ${getChangeColor(stat.changeType)} flex items-center gap-1`}>
                ...
                {typeof stat.change === 'object' ? (stat.change?.label || JSON.stringify(stat.change)) : stat.change}
              </span>
